...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python scripts/step4_gog_download_markdown_and_cancal_img_base64.py <workdir>")
        sys.exit(1)

    directory = Path(sys.argv[1]).expanduser()
    manifest = json.loads((directory / "input_manifest.json").read_text(encoding="utf-8-sig"))
    urls = [str(url).strip() for url in manifest.get("google_doc_url", []) if str(url).strip()]
    ...
    for url in urls:
        ...
        if code != 0:
            ...
            continue
        downloaded.append({"url": url, "file_id": file_id})
